...
import { api, apiCached, invalidateApiCache, API_TOKEN_KEY, API_CACHE_TTL } from './lib/standaloneApi.js';
import { loadAppComponent } from './apps/appLoaders.js';
...
function DeviceProvider({ children }) {
  ...
  const openApp = useCallback((id) => {
    ...
    if (meta?.launchType === 'route' && meta?.launchUrl) {
      window.location.href = meta.launchUrl;
      return;
    }
    if (meta?.launchType === 'external' && meta?.launchUrl) {
  ...
  return (
    <AppShell title="Contact" subtitle="Let's build something memorable.">
      <Card>
        ...
        <Row label="Phone" value="(608) 387-8444" /><Divider />
...
const appViews = {
  ...
  support:SupportApp, music:MusicApp, inventory:InventoryApp,
  'project-a':makeProjectApp('project-a'),
  'project-b':makeProjectApp('project-b'),
  'project-c':makeProjectApp('project-c'),
};

async function resolveAppComponent(app) {
  if (!app) return null;
  const lazy = await loadAppComponent(app.id);
  if (lazy) return lazy;
...
function AppIcon({ app, onTap, showLabel = true, size = APP_TILE_SIZE }) {
  ...
  const handle = () => {
    ...
    if (app.launchType === 'external' && app.launchUrl) {
...
function AppView({ app, isOpen, onClose, onExitComplete }) {
  ...
  useEffect(() => {
    let cancelled = false;
    setViewLoading(true);
    resolveAppComponent(app).then((Comp) => {
      if (!cancelled) {
        setView(() => Comp);
